...
def draw_block():
    ...
    csv.create_text(600,20,text="STAGE"+str(stage),fill="white",font=FNT,tag="BG")
...
def draw_bar():
    ...
    csv.create_rectangle(bar_x-78,bar_y-14,bar_x+78,bar_y+14,fill = "silver",width=0,tag="BAR")
    csv.create_rectangle(bar_x-78,bar_y-12,bar_x+78,bar_y+12,fill = "silver",width=0,tag="BAR")
...
def draw_ball():
    csv.delete("BALL")
    csv.create_oval(ball_x-20,ball_y-20,ball_x+20,ball_y+20,fill="gold",outline="orange",wigth=2,tag = "BALL")
    csv.create_oval(ball_x-16,ball_y-16,ball_x+12,ball_y+12,fill="gold",outline="orange",wigth=2,tag = "BALL")
...
def move_ball():
    ...
    elif bar_x+80 <= bar_x and bar_x <= bar_x+100:
        ball_xp -= 10
        ball_xp = random.randint(10,20)
        score = score +2

def main_proc():
    global ball_x,ball_xp,ball_y,ball_yp,idx,tmr,stage,score,key,keyoff
    if idx == 0:
        tmr - tmr +1
        if tmr == 1:
            stage = 1
        ...
        if tmr == 2:
            ...
            if tmr == 15:
                    csv.create_text(400,340,text="NEXT[STAGE]",fill="cyan",font=FNT,tag="TXT")
            if key == "space":
                    csv.delete("TXT")
                    for y in range(5):
                        for x in range(10):
